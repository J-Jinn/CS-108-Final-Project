...
class GenerationMixin:
    ...
    ##########################################################################################
    # Modified to intercept additional data for use in front-end web app.
    ...
    def _generate_no_beam_search(
        ...
        ##########################################################################################
        # This loop keeps on choosing tokens for predicted text until we reach max iterations.

        next_token_data = {}  # Store data for each token.
        all_tokens_data = {}  # Store data for all tokens.
        all_tokens_data_counter = 0
        ...
        while cur_len < max_length:
            ...
            ##########################################################################################
            # This section below determines each token we use in our prediction.
            ...
            if do_sample:
                ...
                next_token_options = torch.multinomial(probs, num_samples=10).squeeze(1)  # For options.
                next_token = next_token_options[0][0]  # Choose 1st.
                if debug:
                    print(f"Next token log-scores:\n{next_token_logscores}")
                    print(f"Next token probabilities:\n{probs}")
                    print(f"Next token options:{next_token_options}")
            else:
                # Greedy decoding
                next_token = torch.argmax(next_token_logits, dim=-1)
            if debug:
                # Predicted next encoded token.
                print(f"Encoded next token: {next_token}")
                next_token_as_list = next_token.tolist()
                print(f"Encoded next token as list: {next_token_as_list}")
                print(f"Next token logits:\n{next_token_logits}")
                print(f"Next token scores:\n{scores}")
            next_token_data["Scores"] = scores
            next_token_data["Next Token"] = next_token
            next_token_data["Next Token Options"] = next_token_options
            # next_token_data.append(scores)  # Save scores for tokens to choose from.
            # next_token_data.append(next_token)  # Save the next chosen token.
        ...
        if debug:
            # Original prompt + predicted text as individual encoded tokens.
            print(f"Input IDs: {input_ids}")
        generated_data = {"Encoded Tokens": input_ids, "Individual Token Data": all_tokens_data}
        # return input_ids
